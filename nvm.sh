...
nvm_get_checksum_alg() {
  if nvm_has_not_aliased_exec "sha256sum"; then
    nvm_echo 'sha-256'
  elif nvm_has_not_aliased_exec "shasum"; then
    nvm_echo 'sha-256'
  elif nvm_has_not_aliased_exec "sha256"; then
    nvm_echo 'sha-256'
  elif nvm_has_not_aliased_exec "gsha256sum"; then
    nvm_echo 'sha-256'
  elif nvm_has_not_aliased_exec "openssl"; then
    nvm_echo 'sha-256'
  elif nvm_has_not_aliased_exec "bssl"; then
    nvm_echo 'sha-256'
  elif nvm_has_not_aliased_exec "sha1sum"; then
    nvm_echo 'sha-1'
  elif nvm_has_not_aliased_exec "sha1"; then
    nvm_echo 'sha-1'
  elif nvm_has_not_aliased_exec "shasum"; then
    nvm_echo 'sha-1'
  else
...
nvm_checksum() {
  local NVM_CHECKSUM
  if [ -z "${3-}" ] || [ "${3-}" = 'sha1' ]; then
    if nvm_has_not_aliased_exec "sha1sum"; then
